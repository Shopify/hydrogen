...
import {HYDROGEN_VERSION} from './version';
import {
  minifyQuery,
  assertQuery,
  assertMutation,
...
// When serializing GraphQLError to JSON for client consumption,
// the class instance is lost and it becomes a plain JSON object.
// We use toJSON() to get a plain-object representation.
...
/**
 *  Creates a full-featured Storefront API client with internationalization (i18n), caching, and server-side optimizations.
 *
...
export function createStorefrontClient<TI18n extends I18nBase>(
  ...
  if (HYDROGEN_VERSION)
    defaultHeaders['user-agent'] = `Hydrogen ${HYDROGEN_VERSION}`;
