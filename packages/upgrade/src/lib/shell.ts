...
async function hasAliasDefinition(aliasName: string, shell: UnixShell) {
  ...
  try {
    ...
    const result = await execAsync(`grep 'alias ${aliasName}' ${resolveFromHome(filepath)}`)
...
async function shellWriteFile(shell: UnixShell, content: string, append = false) {
  const filepath = getShellAliasDefinitionFile(shell)
  ...
  try {
    await execAsync(`printf ${content} ${append ? '>>' : '>'} ${resolveFromHome(filepath)}`)
    return true
  } catch (error) {
    outputDebug(`Could not create or modify ${filepath}:\n` + (error as Error).stack)
    return false
  }
}

export async function shellWriteAlias(shell: UnixShell, aliasName: string, content: string) {
...
export async function shellRunScript(script: string, shellBin: string) {
  try {
    ...
    outputDebug(`Could not run shell script for ${shellBin}:\n` + (error as Error).stack)
    return false
  }
}
...
export async function getCliCommand(
  ...
  if (pkgManager === 'bun' || pkgManager === 'pnpm' || pkgManager === 'yarn') cli = pkgManager
...
export const monorepoPackagesPath = new URL('../../../', import.meta.url).pathname
export const isHydrogenMonorepo = monorepoPackagesPath.endsWith('/hydrogen/packages/')

export const hydrogenPackagesPath = isHydrogenMonorepo ? monorepoPackagesPath : undefined

export function getSkeletonSourceDir() {
  if (!isHydrogenMonorepo) {
    throw new AbortError('Trying to use skeleton source dir outside of Hydrogen monorepo.', 'Please report this error.')
  }

  console.log('monorepo path', monorepoPackagesPath)
  console.log('skeleoton path', joinPath(dirname(monorepoPackagesPath), 'templates', 'skeleton'))
