...
import {getSkeletonSourceDir} from './lib/shell.js'
...
vi.mock('@shopify/cli-kit/node/ui', async () => {
  const original = await vi.importActual<typeof import('@shopify/cli-kit/node/ui')>('@shopify/cli-kit/node/ui')
...
async function createOutdatedSkeletonPackageJson() {
  ...
  console.log(joinPath(getSkeletonSourceDir(), 'package.json'))
  const packageJson: PackageJson = require(joinPath(getSkeletonSourceDir(), 'package.json'))

  if (!packageJson) throw new Error('Could not parse package.json')
  if (!packageJson?.dependencies) throw new Error('Could not parse package.json dependencies')
  if (!packageJson?.devDependencies) throw new Error('Could not parse package.json devDependencies')
...
function increasePatchVersion(depName: string, deps: Record<string, string>) {
  ...
  if (!depVersion) {
    throw new Error(`No dependency "${depName}" found in: ${Object.keys(deps || {})}`)
  }

  return {
    [depName]: depVersion.replace(/\.(\d+)$/, (_, patchVersion) => `.${Number(patchVersion) + 1}`),
  }
}

describe('upgrade', async () => {
  // Create an outdated skeleton package.json for all tests
  const OUTDATED_HYDROGEN_PACKAGE_JSON = await createOutdatedSkeletonPackageJson()

  describe('checkIsGitRepo', () => {
    it('renders an error message when not in a git repo', async () => {
      await inTemporaryDirectory(async (appPath) => {
        await expect(runUpgrade({appPath})).rejects.toThrowError('git repository')
      })
    })
  })
  ...
  describe('getHydrogenVersion', () => {
    it('throws if no package.json is found', async () => {
      await inTemporaryHydrogenRepo(
        async (appPath) => {
          await expect(runUpgrade({appPath})).rejects.toThrowError('valid package.json')
        },
    ...
    it('throws if no hydrogen version is found in package.json', async () => {
      await inTemporaryHydrogenRepo(
        async (appPath) => {
          await expect(runUpgrade({appPath})).rejects.toThrowError('version in package.json')
        },
        {
          cleanGitRepo: true,
          packageJson: {
    ...
    it('exists when run over a prerelease "next" version', async () => {
      await inTemporaryHydrogenRepo(
        async (appPath) => {
          await expect(runUpgrade({appPath})).rejects.toThrowError('prerelease')
        },
        {
          cleanGitRepo: true,
          packageJson: {
    ...
    it('renders "already in the latest version" success message if no upgrades are available', async () => {
      ...
      await inTemporaryHydrogenRepo(
        async (appPath) => {
          ...
          expect(outputMock.info()).toMatch(/ success.+ latest Hydrogen version/is)
        },
        {
          cleanGitRepo: true,
          packageJson: {
    ...
    it('prompts if a passed target --version is not a valid upgradable version', async () => {
      await inTemporaryHydrogenRepo(
        async (appPath) => {
          ...
          vi.mocked(renderSelectPrompt<Release>).mockImplementationOnce(({choices}) =>
    ...
    it('returns the correct fixes and features for a release range thats outdated', async () => {
      await inTemporaryHydrogenRepo(
        async (appPath) => {
          ...
          // 2023.4.1
          const selectedRelease = releases.find((release) => release.version === '2023.4.1')
    ...
    it('renders a confirmation prompt to continue or return to the previous menu', async () => {
      await inTemporaryHydrogenRepo(
        async () => {
          ...
          // 2023.10.0
          const selectedRelease = releases.find((release) => release.version === '2023.10.0') as (typeof releases)[0]
          ...
          ;[...CUMMLATIVE_RELEASE.features, ...CUMMLATIVE_RELEASE.fixes].forEach((feat) =>
  ...
  describe('displayDevUpgradeNotice', () => {
    it('shows up a notice if Hydrogen is outdated', async () => {
      await inTemporaryHydrogenRepo(
        async (targetPath) => {
          await expect(displayDevUpgradeNotice({targetPath})).resolves.not.toThrow()

          expect(outputMock.info()).toMatch(/new @shopify\/hydrogen versions? available/i)
          expect(outputMock.info()).toMatch(/Current: [\d.] | Latest: [\d.]+\s{2,}/i)
          expect(outputMock.info()).toMatch(/The next \d+ version\(s\) include/i)
    ...
    it('shows up a notice if there are related dependencies to upgrade', async () => {
      const latestHydrogenVersion = (await (await getChangelog()).releases[0]?.version) as string
      ...
      await inTemporaryHydrogenRepo(
        async (targetPath) => {
          ...
          expect(outputMock.info()).toMatch(/new @shopify\/hydrogen versions? available/i)
          expect(outputMock.info()).toMatch(/Current: [\d.] | Latest: [\d.]+ with updated dependencies\s{1,}/i)
          expect(outputMock.info()).toMatch(/The next 1 version\(s\) include/i)
          expect(outputMock.info()).toMatch(`or \`h2 upgrade --version ${latestHydrogenVersion}\``)
        },
        {
          cleanGitRepo: false,
          packageJson: {
