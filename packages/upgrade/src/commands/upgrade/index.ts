...
import {fileExists, isDirectory, mkdir, removeFile, touchFile, writeFile, readFile} from '@shopify/cli-kit/node/fs'
...
import {getCliCommand, hydrogenPackagesPath, isHydrogenMonorepo} from '../../lib/shell.js'
import {flagsToCamelObject} from '../../lib/flags.js'
...
export async function runUpgrade({appPath, version: targetVersion, force}: UpgradeOptions) {
  if (!force) {
  ...
  if (isPrerelease) {
    throw new AbortError('The upgrade command cannot be run over a prerelease Hydrogen version')
  }
  ...
  if (!availableUpgrades?.length) {
    renderSuccess({
      headline: `You are on the latest Hydrogen version: ${getAbsoluteVersion(currentVersion)}`,
    })
...
export async function getChangelog(): Promise<ChangeLog> {
  ...
  if (isHydrogenMonorepo && hydrogenPackagesPath && process.env.FORCE_CHANGELOG_SOURCE !== 'remote') {
    ...
    return require(joinPath(dirname(hydrogenPackagesPath), 'docs', 'changelog-2.json')) as ChangeLog
  }

  try {
    const response = await fetch('https://hydrogen.shopify.dev/changelog-2.json')
  ...
  throw new AbortError('Failed to fetch Hydrogen changelog', 'Ensure you have internet connection and try again')
}
...
function hasOutdatedDependencies({
  ...
  return Object.entries({
    ...
    const isDependencyOutdated = semver.gt(getAbsoluteVersion(version), getAbsoluteVersion(currentDependencyVersion))
...
export async function getSelectedRelease({
  ...
  const targetRelease = targetVersion
    ? availableUpgrades.find((release) => getAbsoluteVersion(release.version) === getAbsoluteVersion(targetVersion))
    : undefined
...
export function getCummulativeRelease({
  ...
  const upgradingReleases = availableUpgrades.filter((release) => {
    const isHydrogenUpgrade =
      semver.gt(release.version, currentPinnedVersion) && semver.lte(release.version, selectedRelease.version)
...
function maybeIncludeDependency({
  ...
  const isRequired = Boolean(selectedRelease.dependenciesMeta?.[name]?.required)
  ...
  const isOlderVersion = semver.lt(getAbsoluteVersion(existingDependencyVersion), getAbsoluteVersion(version))
...
export function buildUpgradeCommandArgs({
  ...
  const selectedRemix = Object.entries(selectedRelease.dependencies).find(isRemixDependency)

  if (currentRemix && selectedRemix) {
    const shouldUpgradeRemix = semver.lt(getAbsoluteVersion(currentRemix[1]), getAbsoluteVersion(selectedRemix[1]))

    if (shouldUpgradeRemix) {
      args.push(...appendRemixDependencies({currentDependencies, selectedRemix}))
    }
  }
 ...
 */
async function promptUpgradeOptions(currentVersion: string, availableUpgrades: Release[]) {
  if (!availableUpgrades?.length) {
...
async function displayUpgradeSummary({
  ...
  const updatedDependenciesList = [
    ...Object.entries(selectedRelease.dependencies || {}).map(([name, version]) => `${name}@${version}`),
    ...Object.entries(selectedRelease.devDependencies || {}).map(([name, version]) => `${name}@${version}`),
  ]
    ...
    customSections: [
      ...
      },
      {
        ...
        body: [
          {
            list: {
              items: [`Run \`git restore . && git clean -df && ${packageManager} i\``],
            },
          },
        ],
      },
 ...
 */
async function validateUpgrade({appPath, selectedRelease}: {appPath: string; selectedRelease: Release}) {
 ...
 */
export async function displayDevUpgradeNotice({targetPath}: {targetPath?: string}) {
  try {
    ...
    const currentReleaseIndex = isLatestHydrogenPackage
      ? changelog.releases.findIndex((release) => getAbsoluteVersion(release.version) !== pinnedCurrentVersion)
      : changelog.releases.findIndex((release) => getAbsoluteVersion(release.version) === pinnedCurrentVersion)
    ...
    let headline =
      Object.keys(uniqueAvailableUpgrades).length > 1
        ? `There are ${Object.keys(uniqueAvailableUpgrades).length} new @shopify/hydrogen versions available.`
          ...
          ? [
              {
                ...
                body: [
                  {
                    list: {
                      items: [...nextReleases, availableUpgrades.length > 5 ? `...more` : ''].flat().filter(Boolean),
                    },
                  },
                ].filter(Boolean),
              },
