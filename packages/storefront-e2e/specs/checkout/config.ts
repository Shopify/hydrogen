...
export type CheckoutTestProduct = {
...
export const test = createTest({
  discover: async ({ storefrontBaseUrl, storefrontClient }) => {
    ...
    abortOnGraphQLErrors("Checkout cart enabled products list", products.errors);

    if (products.data === null) {
      throw new AbortSuiteError("Checkout cart enabled products list returned no data");
    }
    ...
    return { data: { paths: CHECKOUT_PATHS, products: cartEnabledProducts } };
  },
});
