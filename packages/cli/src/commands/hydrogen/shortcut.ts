...
export default class GenerateRoute extends Command {
  ...
  static flags = {
    binPath: Flags.string({
      ...
      env: 'SHOPIFY_HYDROGEN_FLAG_EXECUTABLE_PATH',
    }),
  };
  ...
  async run(): Promise<void> {
    ...
    runCreateShortcut(flags.binPath);
  }
}

export function runCreateShortcut(userBinDir?: string) {
  ...
  if (!globalBinDir) {
    return renderFatalError({
      ...
      tryMessage:
        'Try passing the path to a directory for global executables in the `--executable-path` flag.',
    });
  }
  ...
  fs.copyFileSync(localBinPath, globalBinPath);
  fs.chmodSync(globalBinPath, 0o755); // +x

  renderSuccess({
    headline: 'Shortcut created. You can now run `h2` from anywhere.',
  });
}
