...
export async function isPluginLinked(): Promise<PluginStatus> {
  ...
  try {
    ...
    const isLinked = result.stdout.includes('linked ESM module') || 
...
export async function linkPlugin(pluginPath: string): Promise<LinkResult> {
  try {
    ...
    if (result.stdout.includes('Linked') || result.stdout.includes('already linked')) {
...
export function shouldAutoLink(options: AutoLinkOptions): boolean {
  ...
  if (hasPathFlag && isInsideHydrogenMonorepo()) {
    ...
    if (pathIndex !== -1 && pathIndex < args!.length - 1) {
      ...
      outputDebug(`--path flag detected, target: ${targetPath} - auto-linking for external project workflow`);
      return true;
    }
  }
...
export async function ensureMonorepoPluginLinked(
  ...
  try {
    ...
    if (linkResult.success) {
      ...
      if (process.env.CI === 'true' || process.env.GITHUB_ACTIONS === 'true') {
        outputDebug('Auto-linking failed in CI, but local CLI will be used via npm scripts');
        return true;
      }
      return false;
    }
