 ...
 *
 * YAML catalog entries map package names to version ranges, e.g.:
 *   catalog:
 *     react: '^18.3.1'
 *     '@types/react': '^18.3.28'
 *
 * We use a regex rather than a full YAML parser to avoid adding a
 * dependency just for this test utility. The regex captures all
 * consecutive indented lines after `catalog:`, which is how YAML
 * block mappings work — indentation defines the block boundary.
 */
 ...
 * Glob patterns like `packages/*` are returned as-is — callers must expand
 * them if needed. The Hydrogen monorepo uses explicit paths, not globs.
 */
...
interface ResolveWorkspaceProtocolsOptions {
  ...
  /** Returns the resolved version for a workspace package, or null if not found. */
  resolveWorkspaceVersion: (packageName: string) => Promise<string | null>;
  /** Version to use when a workspace package can't be resolved. */
