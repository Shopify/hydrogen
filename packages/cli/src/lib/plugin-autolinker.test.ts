...
describe('plugin-autolinker', () => {
  ...
  describe('getMonorepoPluginPath', () => {
    ...
    it('should return plugin path if in monorepo and plugin exists', async () => {
      ...
      expect(getMonorepoPluginPath('/monorepo/examples')).toBe('/monorepo/packages/cli');
    });
...
└─ @shopify/cli-hydrogen
   ...
   ├─ location /Users/test/.npm/@shopify/cli-hydrogen`,
    ...
    it('should handle command failure gracefully', async () => {
      const processModule = await import('./process.js');
      vi.spyOn(processModule, 'execAsync').mockRejectedValue(new Error('Command failed'));
  ...
  describe('shouldAutoLink', () => {
    ...
    it('should allow auto-linking for env and deploy commands', async () => {
      ...
      expect(shouldAutoLink({command: 'hydrogen:env:list', workingDirectory: '/monorepo/examples/b2b'})).toBe(true);
      expect(shouldAutoLink({command: 'hydrogen:env:pull', workingDirectory: '/monorepo/examples/b2b'})).toBe(true);
      expect(shouldAutoLink({command: 'hydrogen:env:push', workingDirectory: '/monorepo/examples/b2b'})).toBe(true);
      expect(shouldAutoLink({command: 'hydrogen:deploy', workingDirectory: '/monorepo/examples/b2b'})).toBe(true);
    });
    ...
    it('should return true with --path flag when running from monorepo', async () => {
      vi.spyOn(build, 'isInsideHydrogenMonorepo')
  ...
  describe('ensureMonorepoPluginLinked', () => {
    ...
    it('should skip if shouldAutoLink returns false', async () => {
      const {ensureMonorepoPluginLinked} = await import('./plugin-autolinker.js');
      vi.spyOn(build, 'isInsideHydrogenMonorepo').mockReturnValue(false);
      
      const result = await ensureMonorepoPluginLinked({command: 'hydrogen:dev'});
    ...
    it('should link plugin when not linked', async () => {
      ...
      expect(result).toBe(true);
      expect(console.log).toHaveBeenCalledWith('🔗 Auto-linked local Hydrogen CLI for development');
    });
    ...
    it('should handle errors gracefully', async () => {
      ...
      const processModule = await import('./process.js');
      vi.spyOn(processModule, 'execAsync').mockRejectedValue(new Error('Network error'));
