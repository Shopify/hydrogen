...
export async function setupLiveReload(devServerPort: number, root: string) {
  try {
    const remixRunHmrPath = require.resolve(
    ...
    const remixRunSocketPath = require.resolve(
    ...
    const remixRunHdrPath = require.resolve(
    ...
    const remixRunResultPath = require.resolve(
