...
export function insertCommentBefore(
  ...
  const exists = path.value.comments.find(
    (comment: {value: string}) => comment.value === content,
  );
...
export function insertChildJSXElement(
  j: core.JSCodeshift,
  parent: ASTPath<any>,
  element: string,
  position: 'start' | 'end',
  ...
  const child = j.jsxElement(
    j.jsxOpeningElement(j.jsxIdentifier(element), [], true),
    null,
    [],
  );
...
export function hasChildJSXElement(
  parent: ASTPath<JSXElement>,
  element: string,
) {
  return parent.node.children?.some((child) => {
    return (
      ...
      child.openingElement.name.name === element
    );
  });
}
