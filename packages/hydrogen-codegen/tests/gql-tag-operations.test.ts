...
describe('gql-tag-operations-preset', () => {
  it('can generate simple examples uppercase names', async () => {
    const result = await executeCodegen({
      ...
      documents: path.join(
        __dirname,
        'fixtures/simple-uppercase-operation-name.ts',
      ),
      generates: {
        out1: {
          preset,
  ...
  it('can generate simple examples lowercase names', async () => {
    const result = await executeCodegen({
      ...
      documents: path.join(
        __dirname,
        'fixtures/simple-lowercase-operation-name.ts',
      ),
      generates: {
        out1: {
          preset,
  ...
  it("follows 'useTypeImports': true", async () => {
    ...
    expect(gqlFile.content).toMatchInlineSnapshot(`
      ...
      export type DocumentType<TDocumentNode extends DocumentNode<any, any>> = TDocumentNode extends DocumentNode<  infer TType,  any>  ? TType  : never;"
    `);
    const graphqlFile = result.find(
      (file) => file.filename === 'out1/graphql.ts',
    );
  ...
  describe('fragment masking', () => {
    it('fragmentMasking: true', async () => {
      ...
      `);
      const fragmentMaskingFile = result.find(
        (file) => file.filename === 'out1/fragment-masking.ts',
      );
    ...
    it('can accept null in useFragment', async () => {
      ...
      validateTs(
        ...
        false,
        true,
        [`Duplicate identifier 'DocumentNode'.`],
        true,
      );
    });
  ...
  it('fragmentMasking.augmentedModuleName', async () => {
    ...
    expect(result).toHaveLength(4);
    const fragmentMaskingFile = result.find(
      (file) => file.filename === 'out1/fragment-masking.d.ts',
    );
