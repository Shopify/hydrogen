...
const data: ReferenceEntityTemplateSchema = {
  ...
  description: `The [Storefront API uses cursors](https://shopify.dev/docs/api/usage/pagination-graphql) to paginate through lists of data. The \`<Pagination />\` component makes it easy to render large lists of data from the Storefront API. It is important for pagination state to be maintained in the URL, so that the user can navigate to a product and return back to the same scrolled position in a list. It is also important that the list state is shareable via URL. The \`<Pagination>\` component provides a render prop with properties to load more elements into your list.`,
  type: 'component',
  defaultExample: {
