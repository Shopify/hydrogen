...
export function toArray(tags: CustomHeadTagObject[]): Record<string, string>[] {
  return tags
    ...
    .map((tag) => {
      ...
      if (tag.props.name) {
        obj.name = tag.props.name;
      }

      if (tag.props.property) {
        obj.property = tag.props.property;
      }

      if (tag.props.httpEquiv) {
        obj.httpEquiv = tag.props.httpEquiv;
      }

      obj.content = tag.props.content;

      return obj as V2_HtmlMetaDescriptor;
    });
}
