...
describe('<Analytics.Provider />', () => {
  ...
  describe('useAnalytics()', () => {
    ...
    it('returns prevCart with an updated cart', () => new Promise(done => {
      triggerCartUpdate({
        cart: CART_DATA_2,
        callback: (analytics: AnalyticsContextValue | null) => {
          expect(analytics?.canTrack()).toBe(true);
    ...
    it('can subscribe and receive a page_viewed event', () => new Promise(done => {
      renderAnalyticsProvider({
        cart: CART_DATA,
    ...
    it('can subscribe and receive a product_viewed event', () => new Promise(done => {
      renderAnalyticsProvider({
        cart: CART_DATA,
        registerCallback: (analytics, ready) => {
        ...
        children: <Analytics.ProductView
          data={{
            products: [
              {
                id: 'gid://shopify/Product/6730943823928',
    ...
    it('can subscribe and receive a product_added_to_cart event', () => new Promise(done => {
      triggerCartUpdate({
        cart: CART_DATA_2,
        registerCallback: (analytics, ready) => {
    ...
    it('can subscribe and receive a product_removed_from_cart event', () => new Promise(done => {
      triggerCartUpdate({
        cart: CART_DATA_3,
        registerCallback: (analytics, ready) => {
...
type RenderAnalyticsProviderProps = {
  ...
  registerCallback?: (analytics: AnalyticsContextValue, ready: () => void) => void;
  children?: ReactNode;
}
...
function renderAnalyticsProvider({
  ...
  const AnalyticsProvider = ({updateCart, updateCustomData}: {
...
function LoopAnalytics({children, registerCallback}: {
