...
const data: ReferenceEntityTemplateSchema = {
  ...
  related: [],
  description:
    "A hook that loads the [Customer Privacy API](/docs/api/customer-privacy). When using Hydrogen's request handler, the Storefront API proxy is automatically detected and used for consent requests.\n\nYou can also listen to a `document` event for `shopifyCustomerPrivacyApiLoaded`. It will be emitted when the Customer Privacy API is loaded.",
  type: 'hook',
  defaultExample: {
