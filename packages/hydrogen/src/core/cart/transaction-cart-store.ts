...
type TransactionDefinition<TPayload> = {
  payload: TPayload;
  transport(payload: TPayload, signal: AbortSignal): Promise<UpdateCartResult>;
...
let availableStandardActions: ShopifyStandardActions | null = null;
...
function publishVisibleState(store: CartStoreContext): void {
  const next = reuseVisibleReferences(store.observable.state, projectVisibleState(store));
...
function requireUpdateCart(): ShopifyStandardActions["updateCart"] {
  const updateCart = availableStandardActions?.updateCart;
  if (!updateCart) throw new Error("Standard Actions are not ready.");
  return updateCart;
}
...
export const CART_TRANSACTION_TYPES = defineTransactionTypes({
  ...
  change_line_quantity: {
    ...
    transport: (payload, signal) =>
      requireUpdateCart()(
        { lines: [{ id: payload.lineId, quantity: payload.quantity }] },
        { signal },
      ),
    projectPayload: (state, payload) => {
  ...
  set_discount_codes: {
    ...
    transport: (payload, signal) =>
      requireUpdateCart()({ discountCodes: payload.discountCodes }, { signal }),
    projectPayload: (state, payload) => {
  ...
  set_note: {
    ...
    transport: (payload, signal) => requireUpdateCart()({ note: payload.note }, { signal }),
    projectPayload: (state, payload) => ({
      ...state,
...
function enqueueTransaction<TType extends TransactionType>(
  ...
  const reservation = store.reservation?.type === type ? store.reservation : undefined;
...
async function dispatchTransaction<TType extends TransactionType>(
  ...
  await getShopifyStandardActions();
  ...
  try {
    promise = getTransactionDefinition(type).transport(payload, signal);
  } finally {
    store.reservation = null;
  }

  if (!reservation.consumed) {
    enqueueTransaction(store, type, payload, promise);
  }
...
export function createTransactionCartStore<TData extends CartData = CartData>(
  ...
  const store: CartStoreContext = {
    observable: createObservable<CartState>(settled),
    settled,
    transactions: [],
    projectedErrors: [],
    nextTransactionId: 1,
...
export const createCartStore = createTransactionCartStore;
...
function submitLineIntent(
  ...
  if (configuredCartEndpoint && payload.quantity > 0) {
    syncQuantityInputs([{ id: lineId, quantity: payload.quantity }], configuredCartEndpoint);
  }
  ...
  return (standardActionsPromise ??= new Promise<ShopifyStandardActions>((resolve, reject) => {
    const configure = () => {
      ...
      if (actions?.updateCart) {
        ...
        availableStandardActions = actions;
        resolve(actions);
        return;
      }
