...
describe("shopify scripts", () => {
  ...
  it("mounts Inbox once when enabled", async () => {
    await initializeShopifyScripts({ inbox: true, webMcp: false });
    await initializeShopifyScripts({ inbox: true, webMcp: false });

    expect(document.body.querySelectorAll("shopify-chat")).toHaveLength(1);
  });

  it("does not duplicate an existing Inbox mount", async () => {
    document.body.appendChild(document.createElement("shopify-agent"));

    await initializeShopifyScripts({ inbox: true, webMcp: false });

    expect(document.body.querySelectorAll("shopify-chat")).toHaveLength(0);
    expect(document.body.querySelectorAll("shopify-agent")).toHaveLength(1);
  });

  it("does not mount Inbox when automatic mounting is disabled", async () => {
    await initializeShopifyScripts({ inbox: { autoMount: false }, webMcp: false });

    expect(document.body.querySelector("shopify-chat, shopify-agent")).toBeNull();
  });

  it("does not mount Inbox by default", async () => {
  ...
  it("includes the Inbox module when automatic mounting is disabled", () => {
    const descriptors = getShopifyScriptTags({
      shop: TEST_SHOP,
      inbox: { autoMount: false },
    });
