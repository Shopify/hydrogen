...
import type { AnalyticsCart, AnalyticsCartLine, CartUpdatePayload, ShopAnalytics } from "./types";
...
export function createCartTracker(deps: CartTrackerDeps) {
  ...
  function updateCart(cart: AnalyticsCart | null) {
    ...
    const previousCartLines = flattenConnection<AnalyticsCartLine>(prevCart?.lines);
