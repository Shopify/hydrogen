...
export const PageType = {
  ...
  // Legacy Shopify customer account routes. Included for Monorail parity;
  // most headless storefronts use custom account pages instead.
  ...
  // Shopify platform-rendered pages. Included for Monorail parity;
  // these pages are typically served by Shopify, not the storefront framework.
  captcha: "captcha",
  giftCard: "gift_card",
  forbidden: "403",
  notFound: "404",
...
function schemaWrapper(schemaId: string, payload: MonorailPayload): MonorailEvent {
  return {
    schema_id: schemaId,
    payload,
      ...
      addDataIf(
        ...
        },
        {
          appClientId: payload.shopifySalesChannel
            ? ShopifyAppId[payload.shopifySalesChannel as keyof typeof ShopifyAppId]
...
async function sendToShopify(events: MonorailEvent[], shopDomain?: string): Promise<void> {
...
export function sendShopifyAnalytics(
  ...
  if (eventName === MonorailEventName.PAGE_VIEW_2) {
    events = events.concat(trekkiePageView(payload), customerPageView2(payload));
