  ...
  describe('discoverCustomerAccountEndpoints', () => {
    it('successfully discovers all endpoints', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('Discovered 4/4 Customer Account endpoints'),
      );
    });
    ...
    it('handles partial discovery gracefully', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('Discovered 3/4 Customer Account endpoints'),
      );
    });

    it('returns empty object when no domain is provided', async () => {
      const result = await discoverCustomerAccountEndpoints('');
      expect(result).toEqual({});
      expect(console.warn).toHaveBeenCalledWith(
    ...
    it('returns empty object when all discovery fails', async () => {
      vi.mocked(fetch)
        .mockResolvedValueOnce({
          ok: false,
          status: 404,
      ...
      expect(console.warn).toHaveBeenCalledWith(
        expect.stringContaining('No Customer Account endpoints discovered'),
      );
    });
  });
