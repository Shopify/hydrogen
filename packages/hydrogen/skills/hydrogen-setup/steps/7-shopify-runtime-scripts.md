...
const i18n: ShopifyScriptsI18n = {
  country: requestContext.locale.country, // e.g. "US"
  language: requestContext.locale.language, // e.g. "EN"
  pathPrefix: requestContext.locale.pathPrefix, // "" for the default locale
};
```
...
`i18n` is the same resolved locale used by Storefront API requests: read `country`, `language`, and `pathPrefix` from `requestContext.locale` (or `storefrontClient.locale`), plus optional `currency`. It is not the `defineShopifyI18n` definition, not a locale string, and not the analytics consent config.
