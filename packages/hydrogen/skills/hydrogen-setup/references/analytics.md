...
The bus tracks replay progress by destination name. Re-adding a removed name resumes after the last event that name was delivered, so component remounts and React Strict Mode's development effect replay do not redeliver retained events. A re-added name does receive retained events published while it was removed. A new name starts from the beginning of retained history. Still prefer registering destinations for the page's lifetime; use the returned cleanup when intentionally removing an integration.
