...
4. **Cart events fire** — add an item to the cart. You should see `cart_updated` followed by `product_added_to_cart`. If you see `cart_updated` repeating with the same payload, the dedupe key (`updatedAt`) is missing — confirm the cart data reaching the store includes `updatedAt` (Hydrogen's built-in cart queries select it).
...
- **`updatedAt` missing from cart data breaks dedupe.** The cart tracker keys dedupe on cart `updatedAt` and falls back to the current time when it is absent. Hydrogen's built-in cart queries select it; app-owned cart queries that feed the store must select it too.
