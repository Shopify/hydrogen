...
- Cart mutation handlers require a same-origin `Origin` header, falling back to `Referer` only when `Origin` is absent; otherwise they return `403`. The trusted origin comes from `sessionManager.getSessionOrigin()` when provided, otherwise from the request URL. Behind a trusted proxy, supply the public origin through the session manager; never derive it from untrusted forwarded headers. Browsers send these headers automatically; server-side callers must supply them explicitly.
