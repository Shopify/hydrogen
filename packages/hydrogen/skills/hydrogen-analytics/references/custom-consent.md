...
Once setup resolves, Hydrogen checks Shopify consent and replays allowed events or clears the buffer when denied. Later updates use the existing CTA consent events. This gate covers Hydrogen analytics destinations; independently loaded trackers need their own consent handling.
...
Setup runs once per analytics bus and resolves without a return value. The first callback remains active across component unmounts, remounts, and React Strict Mode. Hydrogen owns the bus for the page's lifetime; component unmounts do not tear down the consent integration.
