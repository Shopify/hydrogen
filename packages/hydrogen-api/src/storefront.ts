...
/**
 *  Creates a full-featured Storefront API client with server-side caching, internationalization (i18n), GraphQL validation, error handling, and request forwarding. Built on top of the low-level `createStorefrontUtilities` client.
 *
...
export function createStorefrontClient<TI18n extends I18nBase>(
  ...
  async function fetchStorefrontApi<T = any>({
    ...
    if (!response.ok) {
      /**
       * The Storefront API might return a string error, or a JSON-formatted
       * `\{error: string\}`. We try both and conform them to a single errors format.
       */
      let errors;
      let bodyText = body;
      try {
