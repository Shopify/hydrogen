      ...
      const handleOutput = (output: string) => {
        ...
        if (!started && output.includes('success')) {
          const resolvedUrl =
            tunnelUrl ||
            localUrl ||
            (this.port !== undefined && this.port !== OS_ASSIGNED_PORT
          ...
          if (!resolvedUrl) {
            ...
            reject(
              new Error(
                `Server ${this.id} started but no URL was emitted in output`,
              ),
            );
            return;
          }
          ...
          setTimeout(() => resolve(resolvedUrl), tunnelUrl ? 5000 : 0);
        }
