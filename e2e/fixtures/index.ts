...
import {StoreCreditUtil} from './store-credit-utils';
...
export {StoreCreditUtil} from './store-credit-utils';
...
>({
  ...
  storeCredit: async ({page}, use) => {
    const storeCredit = new StoreCreditUtil(page);
    await use(storeCredit);
  },
});
