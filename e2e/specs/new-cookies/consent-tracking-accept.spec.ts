...
test.describe('Consent Tracking - Auto-Allowed (Consent Allowed by Default)', () => {
  [true, false].forEach((withBanner) => {
    test(`should set analytics cookies and fire analytics requests immediately when consent is allowed by default when ${withBanner ? 'using' : 'not using'} privacy banner`, async ({
      storefront,
    }) => {
      // Enable privacy banner setting (but banner shouldn't show since consent is auto-allowed)
      await storefront.setWithPrivacyBanner(withBanner);
      ...
      const serverTimingAfterMigration =
        await storefront.getServerTimingValues();
