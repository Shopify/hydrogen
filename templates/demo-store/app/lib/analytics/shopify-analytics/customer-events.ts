...
export function trackCustomerPageView(
  ...
  const shopify = payload.analyticsData;
  const url = new URL(payload.requestData.location);
...
function buildCustomerPayload(payload: any, extraData: any = {}): any {
  ...
  const location = new URL(requestData.location);

  // const [navigation_type, navigation_api] = getNavigationType();
  let formattedData = {
    source: SHOPIFY_ANALYTICS_SOURCE,
