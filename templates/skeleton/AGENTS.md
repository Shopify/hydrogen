...
Until a Shopify store is linked, this project reads [mock.shop](https://mock.shop): a public, auth-free Storefront API backed by mock reference stores. It isn't one store but many, each on its own host with its own catalog.
