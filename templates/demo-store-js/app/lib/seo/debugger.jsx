    ...
    <Panel>
      <>
        ...
        <div className="py-4 px-4 space-y-4">
          {matches.map(({ id, handle, data }, index) => (
            <div key={id}>
        ...
        {Object.entries(tags).map(([label, entries]) => {
          ...
          return (
            <div key={label} className="font-bold text-sm py-4 px-4">
              <div className="font-bold text-xs pb-2">{LABEL_MAP[label]}</div>
              <pre className="overflow-x-scroll whitespace-pre font-mono rounded-sm bg-gray-100  text-[10px] px-4 py-2">
                {label === "LdJson"
                  ? JSON.stringify(entries, null, 2)
                  : entries.map((entry, index) => renderToString(entry) + "\n")}
              </pre>
            </div>
          );
        })}
...
function Item({ pass, value, property }) {
  const icon =
    pass !== false ? (
      <svg
        xmlns="http://www.w3.org/2000/svg"
