...
- New helpers: `matchLocale` (accepts a `Request`, `URL`, or string), `resolveSupportedLocale` (validates an explicitly chosen locale, or its path segment, against the definition and throws `UnsupportedLocaleError` otherwise), `getSupportedLocales`, `getLocalePathSegment` (the segment that identifies a locale in a URL or route param, defined for every locale regardless of routing type), `getLocalizedHref` (rewrites an href into another locale; a path under pathname routing, an absolute URL under domain routing), and `isSameLocale`.
- Static rendering with per-locale routes: put the locale in a `[locale]` route segment, generate its params from `getSupportedLocales(i18n).map(getLocalePathSegment)`, resolve the param with `resolveSupportedLocale(param, i18n)`, and pass the result as `locale` to `createShopifyRequestContext`. A request handler rewrites incoming URLs into that internal shape (domain routing: hostname → segment; pathname routing: unprefixed default → default segment).
