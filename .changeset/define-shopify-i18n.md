...
- **Breaking:** `requestContext.i18n` is now the definition. The resolved `{language, country, pathPrefix}` moved to `requestContext.locale`. `storefrontClient.i18n` and `storefrontClient.locale` mirror both. `@inContext` variable injection reads `requestContext.locale`.
...
- New helpers: `matchLocale` (accepts a `Request`, `URL`, or string), `getSupportedLocales`, `getLocalizedHref` (rewrites an href into another locale; a path under pathname routing, an absolute URL under domain routing), and `isSameLocale`.
