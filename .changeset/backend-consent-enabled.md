...
- **New**: `window.Shopify.customerPrivacy.backendConsentEnabled` is now set to `true` before the Customer Privacy API script loads. This tells the consent library to use the new server-set cookie mode instead of the legacy `_tracking_consent` JS cookie.
