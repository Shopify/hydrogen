...
Destinations now also receive retained events in order, without skips, when an event is published before a pending replay has run or from a destination callback, including across several callbacks of the same destination. This holds as long as the retained backlog plus events published during delivery fit in the 500-event buffer. Previously, either case could move a destination past retained events it had not received yet, and a destination added from a callback could receive the current event twice. Replay also stops as soon as analytics tracking is no longer allowed, rather than finishing the backlog, and callbacks that keep publishing during delivery now log an error instead of hanging the page.
