...
const currentSortValue = computed(() => {
  const s = state.value;
  return s.sortKey === "PRICE"
    ? getSortByValue("PRICE", s.reverse)
...
function filterRemovalHref(filter: ProductFilter): string {
  ...
  return removal === "?" ? props.searchPath : `${props.searchPath}&${removal.slice(1)}`;
}
...
<template>
  <main class="mx-auto max-w-[1480px] px-6 py-16 md:py-20">
    ...
    <header class="max-w-2xl">
      <h1 class="text-6xl font-black tracking-tight md:text-8xl">Search</h1>
      <p v-if="term" class="mt-6 text-base leading-relaxed text-black/70 md:text-lg">
        Results for "{{ term }}"
      </p>
    ...
    <form method="get" action="/search" class="mt-8 flex max-w-xl gap-3">
      ...
      <button
        type="submit"
        class="rounded bg-black px-5 py-2 text-sm font-semibold text-white hover:bg-black/80"
      >
        Search
      </button>
    </form>
    ...
    <!-- Empty state: no term -->
    <p v-if="!term" class="mt-12 text-lg text-black/60">Enter a search term to find products.</p>

    <!-- Browse form (with filters/sort/grid) -->
    <form
      v-else
      v-bind="fProps"
    ...
    >
      ...
      <div class="flex-1">
        ...
        <div v-if="products.length === 0 && !isLoading" class="mt-16 text-center">
          <p class="text-lg text-black/60">No products found for "{{ term }}".</p>
          <NuxtLink
            v-if="hasActiveFilters"
            :to="searchPath"
