...
/** React Router navigation options for collection filter URL updates. */
export function collectionFilterNavigateOptions(searchParams: URLSearchParams) {
  return {
...
export async function queryCollection({
  ...
  handle,
  searchParams,
}: QueryCollectionOptions): Promise<CollectionLoaderData> {
  ...
  const availableFilters: AvailableFilter[] = products.filters.map((f) => ({
    id: f.id,
    label: f.label,
    type: f.type,
