...
function CartErrorBanner() {
  ...
  const visibleLineIds = useMemo(() => new Set(lines.map((l) => l.id)), [lines]);
  ...
  const bannerMessages = [
    ...new Set([
      ...errors.network.map((e) => e.message),
      ...errors.cart.userErrors.map((e) => e.message),
      ...errors.cart.warnings.map((w) => w.message),
      ...errors.note.userErrors.map((e) => e.message),
      ...orphanedLineErrors.map((e) => e.message),
    ]),
  ];
...
export function CheckoutButton() {
  ...
  return checkoutUrl && lines.length > 0 ? (
    <div className="space-y-3">
      <ShopPayButton channel="headless" width="100%" height="48px" borderRadius="4px" />
      <a
        href={checkoutUrl}
