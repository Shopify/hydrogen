...
export class LegacyCustomerAccountUtil {
  ...
  async assertLoginPageLinks() {
    await expect(this.page.getByRole("link", { name: /forgot password/i })).toBeVisible();
    await expect(this.page.getByRole("link", { name: /register/i })).toBeVisible();
  }
  ...
  async assertRecoverPageRendered() {
    ...
    await expect(this.getEmailInput()).toBeVisible();
    await expect(this.page.getByRole("button", { name: /request reset link/i })).toBeVisible();
  }
  ...
  getHeaderAccountLink(): Locator {
    return this.page.getByRole("banner").getByRole("link", { name: /sign in|account/i });
  }
  ...
  async assertAddressesPageRendered() {
    await expect(this.page.getByRole("heading", { name: /addresses/i })).toBeVisible();
  }
