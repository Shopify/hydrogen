...
export class StorefrontPage {
  ...
  private setupRequestTracking() {
    ...
    this.page.on("request", (request) => {
      ...
      if (url.includes(MONORAIL_PRODUCE_URL) && !url.includes(MONORAIL_BATCH_URL)) {
  ...
  private async setupCDPTracking(page: Page) {
    try {
      ...
      cdp.on("Network.requestWillBeSent", (event: any) => {
        ...
        const initiatorUrl =
          event.initiator?.url || event.initiator?.stack?.callFrames?.[0]?.url || "";
        if (initiatorUrl) {
  ...
  /**
   * Assert that legacy JS-visible analytics cookies are absent.
   */
  ...
  async expectEssentialCookiePresent() {
    ...
    const essentialCookie = cookies.find(
      (c) => c.name === "_shopify_essential" || c.name === "_shopify_essentials",
    );

    expect(essentialCookie, "_shopify_essential(s) cookie should be present").toBeDefined();
   ...
   */
  expectPerfKitLoaded() {
    expect(this.perfKitScriptLoaded, "Perf-kit script should be loaded").toBe(true);
  }
  ...
  async navigateToInStockProduct() {
    ...
    throw new Error(
      `No product with a usable cart form found at ${listingUrl} ` +
        `(checked ${productUrls.length} of ${linkCount} product links). ` +
        `Rejected products: ${JSON.stringify(rejectedProducts, null, 2)}`,
    );
  }
  ...
  private async isProductReadyForCart() {
    return expect
      .poll(
        async () => {
          const state = await this.getProductFormState();
          return (
            state.merchandiseId.length > 0 &&
            state.addToCartVisible &&
            state.addToCartText.toLowerCase() === "add to cart" &&
            state.addToCartDisabled === false
          );
        },
        { timeout: 10000 },
      )
  ...
  private async getProductFormState(): Promise<ProductFormState> {
    return this.page.evaluate(() => {
      const buttons = [...document.querySelectorAll("button")];
      const addToCartButton =
        buttons.find((button) => button.textContent?.trim().toLowerCase() === "add to cart") ??
        null;
      const merchandiseInput = document.querySelector<HTMLInputElement>(
      ...
      return {
        ...
        merchandiseId: merchandiseInput?.value ?? "",
        addToCartText: addToCartButton?.textContent?.trim() ?? "",
        addToCartVisible: addToCartButton
          ? Boolean(
              addToCartButton.offsetWidth ||
              addToCartButton.offsetHeight ||
              addToCartButton.getClientRects().length,
            )
          : false,
        addToCartDisabled: addToCartButton ? addToCartButton.disabled : null,
        soldOutTextVisible: buttons.some((button) => button.textContent?.trim() === "Sold out"),
  ...
  /**
   * Add the selected product to cart and wait for the cart route response.
   */
  async addToCart() {
    ...
    const merchandiseIdInput = this.page.locator('input[name="merchandiseId"]').first();
    await expect(merchandiseIdInput).toHaveValue(/.+/, { timeout: 10000 });

    const response = await this.page.evaluate(async () => {
      const merchandiseId = (
        document.querySelector('input[name="merchandiseId"]') as HTMLInputElement | null
      )?.value;

      if (!merchandiseId) {
        throw new Error('Missing "merchandiseId" input value.');
      }
      ...
      return fetch("/api/cart", {
        ...
        text: cartResponse.ok ? "" : await cartResponse.text(),
      }));
    });
    ...
    return this.page.evaluate(() => {
      ...
      if (links.length === 0) {
        ...
        links = document.querySelectorAll(
          '[role="dialog"] a[href*="checkout"], [role="dialog"] a[href*="/cart/c"], [role="dialog"] a[href*="/cart/"]',
        );
      }
   ...
   */
  async verifyCheckoutUrlTrackingParams(expectedY: string, expectedS: string, context: string) {
    const checkoutUrls = await this.getCheckoutUrls();

    expect(checkoutUrls.length, `Should have checkout URLs ${context}`).toBeGreaterThan(0);
    ...
    for (const url of checkoutUrls) {
      ...
      expect(yParam, `Checkout URL should have '_y' param ${context}`).toBe(expectedY);
      expect(sParam, `Checkout URL should have '_s' param ${context}`).toBe(expectedS);
    }
  ...
  async navigateToCart() {
    ...
    const closeButton = this.page.locator('button[aria-label="Close"], .overlay button.close');
  ...
  expectNoMonorailRequests() {
    expect(this.monorailRequests, "No Monorail analytics requests should be made").toHaveLength(0);
  }
  ...
  async waitForMonorailRequests(minCount = 1) {
    await expect
      .poll(() => this.monorailRequests.length, "Monorail analytics requests should be made")
   ...
   */
  verifyMonorailRequests(context: string): void;
  verifyMonorailRequests(expectedY: string, expectedS: string, context: string): void;
  verifyMonorailRequests(expectedYOrContext: string, expectedS?: string, context?: string) {
    const shouldVerifyTrackingValues = expectedS !== undefined && context !== undefined;
    const assertionContext = shouldVerifyTrackingValues ? context : expectedYOrContext;

    const requestsWithData = this.monorailRequests.filter((req) => req.postData);

    expect(
      requestsWithData.length,
      `Monorail requests with data ${assertionContext}`,
    ).toBeGreaterThan(0);
  ...
  async openCartAside(): Promise<void> {
    const cartLink = this.page.getByRole("banner").getByRole("link", { name: "Cart" });
  ...
  /**
   * Set the consent mode by intercepting serialized loader data and dependency
   * bundles. This example hardcodes the mode in app loader data, while
  ...
  async setConsentMode(mode: "default-banner" | "custom-banner" | "no-banner") {
    ...
    const injectConsentMode = async (route: Route) => {
      const response = await route.fetch();
    ...
    await this.page.route("**/@fs/**/hydrogen/dist/**/*.js", injectConsentMode);
    ...
    await this.page.route("**/.vite/deps/@shopify_hydrogen*.js*", injectConsentMode);
  }

  async getCartId() {
    const cartId = await this.getCookie("cart");
    return cartId?.value ? `${CART_ID_PREFIX}${decodeURIComponent(cartId.value)}` : undefined;
  }
  ...
  async applyDiscountCode(code: string): Promise<{ code: string }> {
    ...
    const applyButton = input.locator("..").getByRole("button", { name: "Apply discount code" });
  ...
  async removeDiscountCode(): Promise<void> {
    const removeButton = this.page.locator('button[aria-label="Remove discount"]:visible').first();
  ...
  async getAppliedDiscountCodes(): Promise<string[]> {
    const discountCodeElements = this.page.locator(".cart-discount code:visible");
   ...
   */
  async expectDiscountCodeApplied(code: string, timeout = 10000): Promise<void> {
    const upperCode = code.toUpperCase();
    const codeLocator = this.page.locator(`.cart-discount code:has-text("${upperCode}"):visible`);
   ...
   */
  async expectDiscountCodeRemoved(code: string, timeout = 10000): Promise<void> {
    const upperCode = code.toUpperCase();
    const codeLocator = this.page.locator(`.cart-discount code:has-text("${upperCode}"):visible`);
   ...
   */
  async applyGiftCard(code: string): Promise<{ lastChars: string; amount: string }> {
  ...
  async removeGiftCard(lastCharacters: string): Promise<void> {
    ...
    const cardLocator = this.page.locator(`code:has-text("***${upperLastChars}"):visible`);
   ...
   */
  async getAppliedGiftCards(): Promise<Array<{ lastChars: string; amount: string }>> {
  ...
  private async getAppliedCardAmount(lastCharacters: string): Promise<string> {
    const upperLastChars = lastCharacters.toUpperCase();
    const codeElement = this.page.locator(`code:has-text("***${upperLastChars}"):visible`);
   ...
   */
  async expectGiftCardApplied(lastCharacters: string, timeout = 10000): Promise<void> {
    const upperLastChars = lastCharacters.toUpperCase();
    const cardLocator = this.page.locator(`code:has-text("***${upperLastChars}"):visible`);
   ...
   */
  async expectGiftCardRemoved(lastCharacters: string, timeout = 10000): Promise<void> {
    const upperLastChars = lastCharacters.toUpperCase();
    const cardLocator = this.page.locator(`code:has-text("***${upperLastChars}"):visible`);
   ...
   */
  async expectGiftCardError(expectedPattern: RegExp, timeout = 10000): Promise<void> {
