...
export class CustomerAccountUtil {
  ...
  async login(email: string, otp: string = DEFAULT_OTP) {
    ...
    const otpError = this.page.locator('[role="alert"]').filter({ hasText: /.+/ });
    if (await otpError.isVisible({ timeout: 3000 }).catch(() => false)) {
      throw new Error("OTP submission failed — check if bypass is configured correctly");
    }
  ...
  async logout() {
    ...
    await this.page.waitForURL(
      (url) => !url.hostname.includes("shopify.com") && !url.pathname.startsWith("/account"),
