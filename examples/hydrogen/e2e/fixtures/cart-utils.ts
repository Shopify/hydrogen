...
export class CartUtil {
  ...
  async selectDifferentOption(optionSelect: Locator) {
    ...
    const enabledOptionValues = await optionSelect.evaluate((el: HTMLSelectElement) =>
    ...
    const nextValue = enabledOptionValues.find((value) => value !== initialValue);
    if (!nextValue) {
      throw new Error("Expected option select to have at least two different values");
    }
  ...
  async assertInCart(productName: string) {
    await expect(this.getLineItems().filter({ hasText: productName })).toBeVisible();
  }

  async assertSubtotal(formattedAmount: string) {
    const cartTotals = this.page.getByLabel("Totals");
    const subtotal = cartTotals.getByRole("group").filter({ hasText: "Subtotal" });
    await expect(
  ...
  async assertTotalItems(itemCount: number) {
    await expect(
      this.page.getByRole("link", { name: "Cart" }).getByLabel(`(items: ${itemCount})`),
    ).toBeVisible();
  }

  async setCartId(cartId: string, baseURL?: string) {
    const value = cartId.startsWith(CART_ID_PREFIX) ? cartId.replace(CART_ID_PREFIX, "") : cartId;
  ...
  async navigateToCartPage() {
    ...
    await expect(this.page.getByRole("dialog", { name: "Cart" })).not.toBeVisible();
  }
