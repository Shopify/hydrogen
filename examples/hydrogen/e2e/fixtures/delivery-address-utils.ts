...
export class DeliveryAddressUtil {
  ...
  async navigateToAddresses() {
    ...
    await expect(this.page.getByRole("heading", { level: 2, name: "Addresses" })).toBeVisible();
  }
  ...
  async fillAddressForm(form: Locator, data: Partial<AddressFormData>) {
    for (const [field, label] of Object.entries(DeliveryAddressUtil.FIELD_LABEL_MAP)) {
  ...
  async assertAddressVisible(data: Partial<Omit<AddressFormData, "defaultAddress">>) {
    ...
    throw new Error(`No address found matching ${matchField.label}="${matchField.value}"`);
  }
}
