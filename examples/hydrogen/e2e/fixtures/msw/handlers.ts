    ...
    mockCustomerAccountOperation(CUSTOMER_DETAILS_QUERY, ({ variables }) => {
      return {
        ...
        customer: {
          ...
          firstName:
            variables.language === "FR" ? "Taylor FR" : customerDetailsMock.customer.firstName,
        },
      };
    }),
...
scenarios.set(MSW_SCENARIOS.deliveryAddresses, createDeliveryAddressesScenario());
                ...
                edges: [
                  {
                    node: {
                      ...
                      shippingAddress: {
                        countryCode: "US",
                        formattedAddress: ["123 Main St", "New York, NY 10001", "United States"],
                      },
                    },
                  },
                  {
...
export function getHandlersForScenario(scenario: string | undefined): MswScenarioMeta {
  if (!scenario) {
    return {
      handlers: [],
      mocksCustomerAccountApi: false,
  ...
  if (!meta) {
    throw new Error(`[e2e-msw] Scenario "${scenario}" registered but metadata missing`);
  }
