...
function injectLegacyCustomerSession(session: SessionLike) {
  const current =
    session.get("customerAccessToken") ?? ({} as { accessToken?: string; expiresAt?: string });

  session.set("customerAccessToken", {
    ...current,
    accessToken: current.accessToken ?? "e2e-legacy-customer-access-token",
    expiresAt: current.expiresAt ?? new Date(Date.now() + SESSION_TTL_IN_MS).toISOString(),
  });
}

async function addMockCustomerSessionCookieIfNeeded(request: Request, env: Env) {
...
const appWithMsw = {
  async fetch(request: Request, env: Env, executionContext: ExecutionContext) {
    ...
    const requestWithSession = await addMockCustomerSessionCookieIfNeeded(request, env);
