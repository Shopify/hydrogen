...
export class GiftCardUtil {
  ...
  async assertAppliedCard(lastFourChars: string) {
    ...
    await expect(giftCards.locator("dd").filter({ hasText: `***${lastFourChars}` })).toBeVisible();
  }

  async removeCard(lastFourChars: string) {
    const giftCards = this.page.getByRole("region", { name: "Gift cards" });
    const cardElement = giftCards.locator("dd").filter({ hasText: `***${lastFourChars}` });
    await cardElement
      .getByRole("button", {
  ...
  async assertCardRemoved(lastFourChars: string) {
    const giftCards = this.page.getByRole("region", { name: "Gift cards" });
    await expect(giftCards.locator("dd").filter({ hasText: `***${lastFourChars}` })).toHaveCount(0);
  }
