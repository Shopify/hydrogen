...
const MONOREPO_ROOT = path.resolve(HYDROGEN_EXAMPLE_ROOT, "../..");
...
export const setRecipeFixture = (options: RecipeFixtureOptions) => {
  ...
  const tmpRoot = path.join(HYDROGEN_EXAMPLE_ROOT, ".tmp/recipe-fixtures");
  ...
  const skeletonPath = HYDROGEN_EXAMPLE_ROOT;
