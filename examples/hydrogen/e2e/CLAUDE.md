...
Hydrogen e2e store env files live in `examples/hydrogen/e2e/envs/.env.*` and are
...
**Prerequisites:**
- Shopify CLI authenticated: `cd examples/hydrogen && pnpm exec shopify auth login`
- Skeleton linked to benchmark store: `cd examples/hydrogen && pnpm exec shopify hydrogen link`
