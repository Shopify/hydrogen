...
test.describe("Consent Tracking - Auto-Allowed (Consent Allowed by Default)", () => {
  test("should set analytics cookies and fire analytics requests immediately when consent is allowed by default", async ({
    ...
    storefront.verifyMonorailRequests(tokens.uniqueToken, tokens.visitToken, "after page load");
    ...
    // Keep essential cookies, which persist the saved consent choice.
    ...
    const migrationResponse = await storefront.withConsentResponse(() => storefront.reload());
