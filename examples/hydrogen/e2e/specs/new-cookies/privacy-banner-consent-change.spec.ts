...
test.describe("Privacy Banner - Consent Change", () => {
  test.describe("Accept → Decline", () => {
    test("should stop analytics when user revokes consent", async ({ storefront }) => {
      ...
      storefront.verifyMonorailRequests(originalYValue, originalSValue, "after initial accept");
      ...
      const serverTimingAfterDecline = await storefront.getServerTimingValues(true);
  ...
  test.describe("Decline → Accept", () => {
    test("should start analytics when user grants consent", async ({ storefront }) => {
      ...
      const serverTimingAfterAccept = await storefront.getServerTimingValues(true);
      ...
      storefront.verifyMonorailRequests(newYValue, newSValue, "after reload with consent granted");
    });
  });
});
