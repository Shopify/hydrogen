...
test.describe("Delivery Addresses", () => {
  ...
  test.describe("Read", () => {
    ...
    test("shows the create address form", async ({ addresses }) => {
      ...
      await expect(createForm.getByRole("button", { name: "Create" })).toBeVisible();
    });
  ...
  test.describe("Default Address", () => {
    test("toggles default address to a different address", async ({ addresses }) => {
      const existingForms = addresses.getExistingAddresses();
      ...
      await expect(refreshedForms.first().getByRole("checkbox")).not.toBeChecked();
  ...
  test.describe("Delete", () => {
    ...
    test("shows empty state when all addresses are deleted", async ({ addresses }) => {
