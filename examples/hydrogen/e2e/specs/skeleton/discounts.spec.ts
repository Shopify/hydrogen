...
const ACTIVE_DISCOUNT_CODE = getRequiredSecret("discount_code_active").toUpperCase();
const INACTIVE_DISCOUNT_CODE = getRequiredSecret("discount_code_inactive").toUpperCase();
...
test.describe("Discount codes", () => {
  ...
  test("Removes discount code", async ({ discount }) => {
    ...
    await discount.assertNoDiscounts();
    await discount.assertCodeNotPresent(ACTIVE_DISCOUNT_CODE);
  });
  ...
  test("Shows discount input in cart", async ({ page }) => {
    await expect(page.getByRole("textbox", { name: "Discount code" })).toBeVisible();
    await expect(page.getByRole("button", { name: "Apply discount code" })).toBeVisible();
  });
  ...
  test("Rejects invalid discount code", async ({ discount }) => {
    ...
    await discount.assertNoDiscounts();
    await discount.assertCodeNotPresent(INACTIVE_DISCOUNT_CODE);
  });
