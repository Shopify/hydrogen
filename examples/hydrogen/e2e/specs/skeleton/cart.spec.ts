...
test.describe("Cart", () => {
  test.describe("Line Items", () => {
    ...
    test.describe("Quantity Management", () => {
      ...
      test("disables decrease button at quantity 1", async ({ page }) => {
        const cart = new CartUtil(page);
        const decreaseButton = cart.getDecreaseButton(cart.getLineItems().first());
  ...
  test.describe("Nested Line Items", () => {
    test("Supports nested line items", async ({ page, request }) => {
      ...
      expect(addedLines.data.cartCreate.cart.lines.nodes[0].parentRelationship).toBeNull();
      expect(addedLines.data.cartCreate.cart.lines.nodes[1].parentRelationship).not.toBeNull();
