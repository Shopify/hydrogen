...
// TODO-HYDROGEN-E2E: re-enable after customer-account e2e can run without requiring
// local Shopify CLI tunnel auth, or after CI provides that auth explicitly.
const CUSTOMER_ACCOUNT_E2E_DISABLED = true;
test.skip(
  CUSTOMER_ACCOUNT_E2E_DISABLED,
  "TODO-HYDROGEN-E2E: customer-account tunnel auth is not stable locally.",
);
...
if (CUSTOMER_ACCOUNT_E2E_DISABLED) {
  // Skip registering the dev-server fixture while this suite is disabled.
} else if (externalUrl) {
  if (!externalUrl.startsWith("https://")) {
    throw new Error(`CUSTOMER_ACCOUNT_URL must start with "https://", got: ${externalUrl}`);
  }
...
const testTimeoutInMs = externalUrl ? oxygenDeployTimeoutInMs : TUNNEL_SETUP_TIMEOUT_IN_MS;
...
test.describe("Customer Account", { tag: "@customer-account" }, () => {
  ...
  test.describe("Auth Setup", () => {
    ...
    test("logs in and saves session for reuse", async ({ customerAccount }) => {
      ...
      await customerAccount.saveStorageState(CUSTOMER_ACCOUNT_STORAGE_STATE_PATH);
    });
  });
  ...
  test.describe("Login", () => {
    test("logs in with OTP bypass and shows account page", async ({ customerAccount }) => {
      await customerAccount.login(testEmail);
  ...
  test.describe("Auth Persistence", () => {
    ...
    test("stays logged in after navigating away and back", async ({ page, customerAccount }) => {
  ...
  test.describe("Account Pages", () => {
    ...
    test("navigates to orders page", async ({ page }) => {
      ...
      await expect(page.getByRole("group", { name: /filter orders/i })).toBeVisible();
    });

    test("navigates to profile page", async ({ page }) => {
      await page.goto("/account/profile");

      await expect(page.getByRole("heading", { level: 2, name: /my profile/i })).toBeVisible();
    });

    test("navigates to addresses page", async ({ page }) => {
      await page.goto("/account/addresses");

      await expect(page.getByRole("heading", { level: 2, name: /addresses/i })).toBeVisible();
    });
  });
