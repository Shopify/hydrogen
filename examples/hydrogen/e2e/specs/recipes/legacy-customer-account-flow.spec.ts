...
test.describe("Legacy Customer Account Flow Recipe", () => {
  test.describe("Login Page", () => {
    ...
    test("renders login form with email and password fields", async ({ legacyCustomerAccount }) => {
      await legacyCustomerAccount.assertLoginPageRendered();
    });

    test("shows links to register and forgot password", async ({ legacyCustomerAccount }) => {
      await legacyCustomerAccount.assertLoginPageLinks();
    });

    test("navigates to register page via link", async ({ page, legacyCustomerAccount }) => {
  ...
  test.describe("Register Page", () => {
    ...
    test("navigates to login page via link", async ({ page, legacyCustomerAccount }) => {
  ...
  test.describe("Password Recovery Page", () => {
    ...
    test("renders recovery form with email field", async ({ legacyCustomerAccount }) => {
      await legacyCustomerAccount.assertRecoverPageRendered();
    });
  ...
  test.describe("Unauthenticated Redirects", () => {
    test("redirects /account to /account/login when not logged in", async ({ page }) => {
      await page.goto("/account");
      await expect(page).toHaveURL(/\/account\/login/);
    });

    test("redirects /account/orders to /account/login when not logged in", async ({ page }) => {
      await page.goto("/account/orders");
      await expect(page).toHaveURL(/\/account\/login/);
    });

    test("redirects /account/profile to /account/login when not logged in", async ({ page }) => {
      await page.goto("/account/profile");
      await expect(page).toHaveURL(/\/account\/login/);
    });

    test("redirects /account/addresses to /account/login when not logged in", async ({ page }) => {
  ...
  test.describe("Header Navigation", () => {
    test("header contains an account link", async ({ page, legacyCustomerAccount }) => {
      await page.goto("/");
