...
test.describe("Legacy Customer Account Flow — Authenticated", () => {
  test.describe("Account Home Redirect", () => {
    test("redirects /account to /account/orders when logged in", async ({ page }) => {
  ...
  test.describe("Authenticated Redirects on Public Routes", () => {
    test("redirects /account/login to /account when already authenticated", async ({ page }) => {
      await page.goto("/account/login");
      await expect(page).toHaveURL(/\/account/);
      await expect(page).not.toHaveURL(/\/account\/login/);
    });

    test("redirects /account/register to /account when already authenticated", async ({ page }) => {
      await page.goto("/account/register");
      await expect(page).toHaveURL(/\/account/);
      await expect(page).not.toHaveURL(/\/account\/register/);
    });

    test("redirects /account/recover to /account when already authenticated", async ({ page }) => {
  ...
  test.describe("Orders Page", () => {
    ...
    test("renders welcome heading with customer first name", async ({ legacyCustomerAccount }) => {
      await expect(
    ...
    test("displays account navigation menu", async ({ legacyCustomerAccount }) => {
  ...
  test.describe("Profile Page", () => {
    ...
    test("shows marketing preferences checkbox", async ({ legacyCustomerAccount }) => {
    ...
    test("shows password change section", async ({ legacyCustomerAccount }) => {
      ...
      await expect(legacyCustomerAccount.getNewPasswordConfirmInput()).toBeVisible();
    });
  });
  ...
  test.describe("Addresses Page", () => {
    ...
    test("renders addresses page with heading", async ({ legacyCustomerAccount }) => {
      await legacyCustomerAccount.assertAddressesPageRendered();
    });

    test("shows new address form with required fields", async ({ legacyCustomerAccount }) => {
