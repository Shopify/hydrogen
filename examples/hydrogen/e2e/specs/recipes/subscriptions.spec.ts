...
test.describe("Subscriptions Recipe", () => {
  test.describe("Product Page - Subscription Product", () => {
    ...
    test("displays selling plan selector with subscription options", async ({ page }) => {
      await expect(
        page.getByRole("heading", { level: 3, name: "Subscription Options" }),
      ).toBeVisible();

      const deliveryOption = page.getByRole("link").filter({ hasText: /deliver every/i });
    ...
    test("displays subscription frequency options with links", async ({ page }) => {
      ...
      await expect.poll(() => new URL(page.url()).searchParams.get("selling_plan")).toBeTruthy();
    });

    test("allows switching between subscription frequencies", async ({ page }) => {
      const monthlyOption = page.getByRole("link", {
        name: /deliver every month/i,
      });
      const weeklyOption = page.getByRole("link", {
      ...
      await expect
        .poll(() => {
          const sellingPlan = new URL(page.url()).searchParams.get("selling_plan");
          return sellingPlan !== null;
        })
        .toBe(true);
      const monthlySellingPlan = new URL(page.url()).searchParams.get("selling_plan");
      ...
      await expect
        ...
        .toBe(true);
      const weeklySellingPlan = new URL(page.url()).searchParams.get("selling_plan");
  ...
  test.describe("Cart - Subscription Product", () => {
    test("displays subscription details in cart line item", async ({ page }) => {
      ...
      await expect(firstLineItem.getByText(/deliver every week/i)).toBeVisible();
    });
  });

  test.describe("Product Page - Regular Product", () => {
    test("does not display subscription options on non-subscription products", async ({ page }) => {
