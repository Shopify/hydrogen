import { test, expect, setRecipeFixture, MSW_SCENARIOS, B2B_COMPANY_NAME } from "../../fixtures";
...
test.describe("B2B Recipe", () => {
  test.describe("Location Selector", () => {
    ...
    test("displays all company locations with addresses", async ({ page, b2b }) => {
      await page.goto("/");
    ...
    test("shows change location button in header after selection", async ({ page, b2b }) => {
      await page.goto("/");
    ...
    test("reopens modal when change location button is clicked", async ({ page, b2b }) => {
      await page.goto("/");
  ...
  test.describe("Product Page with B2B Context", () => {
    ...
    test("product page loads and renders add-to-cart button", async ({ page, storefront }) => {
