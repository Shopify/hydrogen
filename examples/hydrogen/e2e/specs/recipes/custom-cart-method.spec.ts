...
test.describe("Custom Cart Method Recipe", () => {
  test.describe("Cart Line Item Variant Selector", () => {
    ...
    test("displays product option selectors in cart line items", async ({ page }) => {
      const cart = new CartUtil(page);
      await expect(cart.getLineItems()).toHaveCount(1);

      const firstLineItem = cart.getFirstLineItem();
      const optionSelects = await variantSelector.waitForOptionSelectors(firstLineItem);
      const minimumExpectedOptions = 2;
      expect(await optionSelects.count()).toBeGreaterThanOrEqual(minimumExpectedOptions);
    });

    test("changes product variant and updates cart line item", async ({ page }) => {
      const cart = new CartUtil(page);
      const firstLineItem = cart.getFirstLineItem();
      ...
      const optionSelect = (await variantSelector.waitForOptionSelectors(firstLineItem)).first();
      const { optionName, nextValue } = await variantSelector.selectDifferentOption(optionSelect);
    ...
    test("maintains single line item when changing variants", async ({ page }) => {
      const cart = new CartUtil(page);
      const firstLineItem = cart.getFirstLineItem();
