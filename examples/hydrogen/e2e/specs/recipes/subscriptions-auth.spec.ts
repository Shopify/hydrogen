...
test.describe("Subscriptions Recipe", () => {
  test.describe("Account Subscriptions Page", () => {
    ...
    test("shows active and cancelled status badges", async ({ page }) => {
      ...
      await expect(subscriptions.getStatusBadgeByText("CANCELLED")).toBeVisible();
    });
    ...
    test("shows cancel button only for active subscriptions", async ({ page }) => {
      const subscriptions = new SubscriptionsUtil(page);
      ...
      await expect(subscriptions.getCancelButton(cancelledRow)).not.toBeVisible();
    });
    ...
    test("cancel button submits and shows cancelling state", async ({ page }) => {
      ...
      await expect(subscriptions.getCancellingButton(activeRow).or(cancelButton)).toBeVisible();
    });
  });
  ...
  test.describe("Account Navigation", () => {
    ...
    test("Subscriptions link navigates to subscriptions page", async ({ page }) => {
      const subscriptions = new SubscriptionsUtil(page);
      await page.goto("/account");
