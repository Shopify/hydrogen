...
test.describe("Bundles Recipe", () => {
  test.describe("Product Cards", () => {
    test("displays bundle badges on product cards in collection grids", async ({ page }) => {
  ...
  test.describe("Bundle Product Page", () => {
    test.beforeEach(async ({ page }) => {
      ...
      await expect(page.getByRole("heading", { level: 1, name: KNOWN_BUNDLE.name })).toBeVisible();
    });
    ...
    test("displays bundled products section", async ({ page }) => {
      ...
      const bundledProductLink = page.getByRole("link", { name: /Qty:\s*\d+/ }).first();
      await expect(bundledProductLink).toBeVisible();
    });

    test('shows "Add bundle to cart" text on add to cart button', async ({ page }) => {
  ...
  test.describe("Cart Flow", () => {
    test("displays bundle badge in cart line items after adding bundle", async ({ page }) => {
      ...
      await expect(lineItems.first().getByText("BUNDLE", { exact: true })).toBeVisible();
    });
  });

  test.describe("Regular Product Page", () => {
    test("does not display bundle-specific UI on regular products", async ({ page }) => {
      ...
      await expect(page.getByRole("heading", { name: "Bundled Products" })).toHaveCount(0);

      await expect(page.getByRole("button", { name: "Add to cart" })).toBeVisible();

      await expect(page.getByRole("button", { name: /add bundle to cart/i })).toHaveCount(0);
    });

    test("does not display bundle badge in cart for regular products", async ({ page }) => {
      const cart = new CartUtil(page);
      ...
      const lineItems = cart.getLineItems();
      await expect(lineItems.first()).toBeVisible();
      await expect(lineItems.first().getByText("BUNDLE", { exact: true })).toHaveCount(0);
    });
  });
});
