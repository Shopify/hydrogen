...
/**
 * The [Storefront API uses cursors](https://shopify.dev/docs/api/usage/pagination-graphql) to paginate
 * through lists of data and the `<Pagination />` component makes it easy to paginate data from the
 * Storefront API. It is a near-literal port of Hydrogen's `Pagination`; the only notable difference is
 * hydration detection, which uses `useHydrated` (a `useSyncExternalStore` hook) instead of a `useEffect`
 * that writes to a `window` global.
 */
export function Pagination<NodesType>({
  ...
  const state = useMemo(
    ...
    [endCursor, hasNextPage, hasPreviousPage, startCursor, nodes, namespace, location.state],
  );

  const NextLink = useMemo(
    () =>
      forwardRef<HTMLAnchorElement, Omit<LinkProps, "to">>(function NextLink(props, ref) {
        return hasNextPage
          ? createElement(Link, {
  ...
  const PreviousLink = useMemo(
    () =>
      forwardRef<HTMLAnchorElement, Omit<LinkProps, "to">>(function PrevLink(props, ref) {
...
function getParamsWithoutPagination(paramsString?: string, state?: PaginationState<unknown>) {
 ...
 * `useSyncExternalStore` gives us the server/client split without touching a `window`
 * global or racing a `useEffect`. On soft navigations the getSnapshot value (`true`)
 * is used immediately, so accumulated pages from history state are picked up.
 */
...
export function usePagination<NodesType>(
  connection: Connection<NodesType>,
  namespace: string = "",
): Omit<PaginationInfo<NodesType>, "isLoading" | "state" | "NextLink" | "PreviousLink"> & {
  startCursor: Maybe<string> | undefined;
  endCursor: Maybe<string> | undefined;
} {
  ...
  const nodes = useMemo(() => {
    if (!isHydrated || !state?.pagination?.[namespace]?.nodes) {
      return flattenConnection(connection);
    }

    if (isPrevious) {
      return [...flattenConnection(connection), ...(state.pagination[namespace].nodes || [])];
    } else {
      return [...(state.pagination[namespace].nodes || []), ...flattenConnection(connection)];
    }
  }, [state, connection, namespace, isHydrated, isPrevious]);

  const currentPageInfo = useMemo(() => {
    const stateInfo = state?.pagination?.[namespace]?.pageInfo;

    let pageStartCursor =
      !isHydrated || stateInfo?.startCursor === undefined
        ? connection.pageInfo.startCursor
        : stateInfo.startCursor;

    let pageEndCursor =
      !isHydrated || stateInfo?.endCursor === undefined
        ? connection.pageInfo.endCursor
        : stateInfo.endCursor;

    let previousPageExists =
      !isHydrated || stateInfo?.hasPreviousPage === undefined
        ? connection.pageInfo.hasPreviousPage
        : stateInfo.hasPreviousPage;

    let nextPageExists =
      !isHydrated || stateInfo?.hasNextPage === undefined
  ...
  const previousPageUrl = useMemo(() => {
    ...
    if (currentPageInfo.startCursor) {
      params.set(cursorParam, currentPageInfo.startCursor);
    }
    return `?${params.toString()}`;
  }, [search, currentPageInfo.startCursor, cursorParam, directionParam]);
  ...
  const nextPageUrl = useMemo(() => {
    ...
    return `?${params.toString()}`;
  }, [search, currentPageInfo.endCursor, cursorParam, directionParam]);

  return { ...currentPageInfo, previousPageUrl, nextPageUrl, nodes };
}

export type { Connection as PaginationConnection };
