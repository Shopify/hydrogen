...
function createCSPHeader(nonce: string, props?: CreateContentSecurityPolicy & ShopProp): string {
  const { shop, ...directives } = props ?? {};
  const nonceString = `'nonce-${nonce}'`;
  const styleSrc = ["'self'", "'unsafe-inline'", "https://cdn.shopify.com"];
  const connectSrc = ["'self'", "https://cdn.shopify.com/", "https://monorail-edge.shopifysvc.com"];
  ...
  for (const key in defaultDirectives) {
    ...
    if (key && directive) {
      combinedDirectives[key] = addCspDirective(directive, defaultDirectives[key]);
    }
  }
