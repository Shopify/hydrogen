...
const getHomeData = createServerFn({ method: "GET" }).handler(async ({ context }) => {
  const { data, errors } = await context.storefrontClient.graphql(HOME_QUERY, {
    cache: Cache.short(),
  });
...
function BestSellers({ products }: { products: HomeData["featuredProducts"] }) {
  return (
...
function ShopByCategory({ collections }: { collections: HomeData["featuredCollections"] }) {
  return (
