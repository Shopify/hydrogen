 ...
 */
const getAccount = createServerFn({ method: "GET" })
  .validator((loginFailed: boolean) => loginFailed)
  .handler(async ({ data: loginFailed, context }): Promise<AccountData> => {
    ...
    if (!customerAccount?.available) {
      return { available: false, customer: null, error: null, loginFailed: false };
    }
    ...
    try {
      ...
      if (!accessToken) {
        return { available: true, customer: null, error: null, loginFailed };
      }
      ...
      if (errors?.length) {
        ...
        return {
          available: true,
          customer: null,
          error: errors[0]?.message ?? "Customer Account API request failed. Try again later.",
          loginFailed,
        };
      }
      ...
      console.error("[hydrogen] Customer Account request failed", error);
      setResponseStatus(502);
      return {
        available: true,
        customer: null,
...
export const Route = createFileRoute("/account")({
  validateSearch: (search): { login?: string } =>
    typeof search.login === "string" ? { login: search.login } : {},
  loaderDeps: ({ search }) => ({ loginFailed: search.login === "failed" }),
  loader: ({ deps }) => getAccount({ data: deps.loginFailed }),
  head: () => ({ meta: [{ title: "Account — CORE" }] }),
  ...
  return (
    ...
    >
      ...
      <p className="type-body text-on-surface-secondary mt-3 max-w-xl">
        This example is running against mock.shop, which has no Customer Account API. Configure real
        store credentials and run over HTTPS to enable login:
      </p>
      <ul className="type-body-sm text-on-surface-secondary mt-4 list-disc ps-6">
        <li>
          <code className="bg-surface-secondary rounded-sm px-1.5 py-0.5">pnpm https:setup</code>
        </li>
        <li>
          <code className="bg-surface-secondary rounded-sm px-1.5 py-0.5">
            pnpm --filter @shopify/hydrogen-example-tanstack-start https:dev
          </code>
        </li>
      </ul>
    </section>
...
function CustomerCard({
  ...
  return (
    ...
    >
      ...
      {/* Plain HTML <form>: /account/logout is a Hydrogen-owned
          ...
          A full-page POST lets the browser follow the redirect natively. The
          login link also uses a full document navigation for the same reason. */}
