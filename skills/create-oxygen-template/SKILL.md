...
4. Add Oxygen/MiniOxygen support:
   ...
   - `@shopify/cli` only for the deploy script. Keep the version declared in the template manifest; it must support the explicit `--assets-dir` and `--worker-dir` flags.
...
`workspace:*` with the exact published preview version before generating the standalone lockfile. The release
compiler validates the version format; the package must resolve from the registry with an integrity hash.
...
The source template's `packageManager` matches the repository root so local development uses the root lockfile. It
does not commit a template lockfile; the root `.gitignore` keeps source-template lockfiles out of this repository.

During distribution, `scripts/preview-template-dist.ts` changes the template to its configured npm version, replaces
`workspace:*` with the exact published preview version, and removes any stale lockfile. The release workflow then
generates `package-lock.json` in an isolated copy. Oxygen requires that generated lockfile for `npm ci`. Verify its
...
7. For distribution validation, copy the template to a temporary directory, replace `workspace:*` with the exact
   published preview version, generate `package-lock.json`, and verify Hydrogen resolves to a registry tarball with
   integrity. Leave the source template lockfile-free.
