...
description: >
  Use when writing, reviewing, or debugging Playwright tests in this repository.
  Applies to e2e specs, Playwright fixtures, locator strategy, wait/assertion
  patterns, test group prerequisite/matcher checks, and Hydrogen storefront test flows.
---
...
Prefer `beforeEach` for repeated deterministic setup. Do not force shared setup when test-group discovery or matcher-based skipping gives clearer failure output, especially in portable storefront contract tests.
...
test.describe('Quantity Management', () => {
  ...
  // other tests that require add to cart
});
...
Test IDs are acceptable when they disambiguate duplicated UI that is already semantically valid, such as cart page versus cart drawer rendering at the same time. Do not add test IDs to avoid fixing missing accessible names.
...
This applies when testing app or example code we own. For portable tests against an already-running merchant/storefront URL, report the missing semantic contract instead of changing the target app.
...
Avoid `page.waitForResponse()` for ordinary UI behavior. It can be appropriate when the network request itself is the feature contract, such as analytics, consent, or performance instrumentation tests.
...
This section is Hydrogen-skeleton-specific. Verify local paths and commands against the current repo before using them.
