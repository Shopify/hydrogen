...
export async function action({request, context, params}: ActionArgs) {
  ...
  return linesRemoveResponse(lineIds, cart, errors, formData);
}
...
*/
export const LinesRemoveForm = forwardRef(
  (
...
/*
  hooks ----------------------------------------------------------------
*/
export function useLinesRemove(
  onSuccess: (event: LinesRemoveEvent) => void = () => {},
) {
  const fetcher = useFetcher();
...
export function useOptimisticLinesRemove(lines?: CartLine[]) {
  const fetchers = useFetchers();
  const linesRemoveFetcher = fetchers.find(
    (fetcher) => fetcher?.submission?.action === ACTION_PATH,
  );
...
export function useOptimisticLineRemove(line?: CartLine) {
...
export async function linesRemoveMutation({
  cartId,
  lineIds,
  params,
  context,
