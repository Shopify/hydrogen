...
they use) is inlined into that file's `<style>` block between the
`AUTO-GENERATED` markers — exactly like every other reference page. There is no
auto-sync script in-tree: edit `tokens.css` and mirror the change by hand into each
inlined copy. Verified at desktop (1280) —
