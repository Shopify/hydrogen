...
> Development experiment for exercising Hydrogen APIs in Astro. This is not a starter template.
> See [`../README.md`](../README.md) for context.

Port of the canonical `experiments/core` design to [Astro](https://astro.build/) running on the Node SSR adapter.
...
- On-demand SSR via `@astrojs/node` (`output: "server"`), matching the data path of the React Router and Next.js templates.
...
| `/collections/:handle` | `experiments/core/reference/collection.html` | Live: queries `collection(handle:)` with up to 24 products |
| `/products/:handle` | `experiments/core/reference/product.html` | Live: gallery, options (Size + Color swatches), add-to-cart UI; "You may also like" pulls from `products(first: 4)` |
| `/blogs/news` | (new) | Live: first article rendered as featured, rest in a 2-col grid |
| `/blogs/news/:handle` | (new) | Live: article body comes back as `contentHtml` and is rendered with `set:html` |
...
```sh
pnpm --filter @shopify/hydrogen-experiment-astro dev:https
```
...
# from the repo root
pnpm install
pnpm --filter @shopify/hydrogen-experiment-astro dev
```
...
```sh
pnpm --filter @shopify/hydrogen-experiment-astro build
pnpm --filter @shopify/hydrogen-experiment-astro start
```
