# SolidStart experiment

> Development experiment for exercising Hydrogen APIs in SolidStart. This is not a starter template.
> See [`../README.md`](../README.md) for context.

Port of the canonical `experiments/core` design to [SolidStart](https://start.solidjs.com/) v1 (Vinxi + Solid Router, SSR enabled by default).
...
| Route                  | Source                                               | Status                                                                                      |
| ---------------------- | ---------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| `/`                    | `experiments/core/reference/home.html`                  | Live: hero is static; featured products grid pulls from `products(first: 3)`                |
| `/collections`         | `experiments/core/reference/collections.html`           | Live: queries `collections(first: 12)`                                                      |
| `/collections/:handle` | `experiments/core/reference/collection.html`            | Live: queries `collection(handle:)` with filters, sort, and active filter chips             |
| `/products/:handle`    | `experiments/core/reference/product.html`               | Live: gallery, URL-synced variant options, add-to-cart, Shop Pay, and related products      |
| `/cart`                | `(Hydrogen primitive)`                                | Live: optimistic line updates, discount codes, order note, totals, checkout, and Shop Pay   |
| `/blogs/news`          | (new)                                                 | Live: featured article + grid from `blog(handle:"news").articles`                           |
| `/blogs/news/:handle`  | (new)                                                 | Live: title, date, `contentHtml` rendered via `innerHTML`                                   |
...
The account flow uses `createCustomerSession` and `createCustomerAccountServerHandlers` from `@shopify/hydrogen/customer-account`, Customer Account values from `experiments/shared/config.ts`, and an encrypted HttpOnly `__Host-` cookie adapter from `experiments/shared/customer-session.ts`.
...
```sh
pnpm --filter @shopify/hydrogen-experiment-solid-start dev:https
```
...
# from the repo root
pnpm install
pnpm --filter @shopify/hydrogen-experiment-solid-start dev
```
...
- `formatMoney()` in `src/lib/money.ts` — every experiment will need money formatting from a `MoneyV2`-shaped object.
...
- **Framework bindings.** This project intentionally uses core primitives directly. If Solid bindings are added later, compare them against `src/lib/cart.tsx`, `src/components/CollectionBrowser.tsx`, and `src/components/ProductPurchasePanel.tsx`.
