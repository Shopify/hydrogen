...
### Worktree Env Fixtures

Hydrogen e2e store env files live in `experiments/hydrogen/e2e/envs/.env.*` and are
intentionally tracked, matching upstream `~/src/github.com/Shopify/hydrogen/e2e/envs`.
If they are missing in a worktree, restore them from Git before running e2e.
...
**Prerequisites:**
- Shopify CLI authenticated: `cd experiments/hydrogen && pnpm exec shopify auth login`
- Skeleton linked to benchmark store: `cd experiments/hydrogen && pnpm exec shopify hydrogen link`
