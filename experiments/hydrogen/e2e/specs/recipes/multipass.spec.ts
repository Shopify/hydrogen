import { test, expect, setRecipeFixture, getRequiredSecret } from "../../fixtures";
...
test.describe("Multipass Recipe", () => {
  ...
  test.describe("Unauthenticated Redirects", () => {
    ...
    test("redirects /account/orders to login when not logged in", async ({ page }) => {
      await page.goto("/account/orders");
      await expect(page).toHaveURL(/\/account\/login/);
    });

    test("redirects /account/profile to login when not logged in", async ({ page }) => {
      await page.goto("/account/profile");
      await expect(page).toHaveURL(/\/account\/login/);
    });

    test("redirects /account/addresses to login when not logged in", async ({ page }) => {
      await page.goto("/account/addresses");
      await expect(page).toHaveURL(/\/account\/login/);
    });
  });
  ...
  test.describe("Header Navigation", () => {
    ...
    test("account link navigates to account area when clicked", async ({ page }) => {
  ...
  test.describe("Multipass Checkout Button", () => {
    test("checkout renders as a button instead of a link after adding to cart", async ({
      ...
      await expect(page.getByRole("link", { name: /continue to checkout/i })).not.toBeVisible();
    });
  });
});
