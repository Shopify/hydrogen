...
test.describe("Combined Listings Recipe", () => {
  test.describe("Combined Listing Product Page", () => {
    ...
    test("variant selection navigates to child product with options", async ({ page }) => {
      const variantLink = page
        .getByRole("main")
        .getByRole("link", { name: KNOWN_COMBINED_LISTING.knownVariant });
      await expect(variantLink).toBeVisible();
      ...
      await expect
        .poll(() => {
          ...
          return (
            url.searchParams.has(KNOWN_COMBINED_LISTING.variantOptionName) &&
            !url.pathname.includes(KNOWN_COMBINED_LISTING.handle)
          );
        })
        .toBe(true);
    });
  ...
  test.describe("Collection Page", () => {
    test("parent combined listing products are filtered from collections", async ({ page }) => {
      await page.goto("/collections/all");

      const regularProductLink = page.getByRole("link", {
        name: KNOWN_PRODUCT_IN_COLLECTION.name,
      });
  ...
  test.describe("Regular Product Page", () => {
    ...
    test('shows "Add to cart" button on regular products', async ({ page }) => {
      const main = page.getByRole("main");
      await expect(main.getByRole("group", { name: "Price" })).toBeVisible();
