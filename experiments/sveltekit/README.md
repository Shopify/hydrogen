...
> Development experiment for exercising Hydrogen APIs in SvelteKit. This is not a starter template.
> See [`../README.md`](../README.md) for context.

Port of the canonical `experiments/core` design to [SvelteKit 2](https://svelte.dev/docs/kit) with [Svelte 5](https://svelte.dev/docs/svelte) (runes mode).
...
| `/collections/:handle` | `experiments/core/reference/collection.html` | Live: queries `collection(handle:)` with up to 24 products, URL-synced filters, sort, and active filter chips |
...
| `/blogs/news` | (new) | Live: `blog(handle:"news") { articles(first: 10) }`, first article rendered as featured |
| `/blogs/news/:handle` | (new) | Live: `articleByHandle`, `contentHtml` rendered with `{@html}` |
...
```sh
pnpm --filter @shopify/hydrogen-experiment-sveltekit dev:https
```
...
# from the repo root
pnpm install
pnpm --filter @shopify/hydrogen-experiment-sveltekit dev
```
...
- `formatMoney()` in `src/lib/money.ts` — every experiment needs money formatting from a `MoneyV2`-shaped object.
...
- **Framework bindings**: this project intentionally uses core primitives directly. If Svelte bindings are added later, compare them against `src/lib/cart.ts`, `src/lib/collection.ts`, and `src/lib/components/ProductPurchasePanel.svelte`.
