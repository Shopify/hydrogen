...
((self) => {
  ...
  const getPartytownScript = () =>
    `<script src="${partytownLibUrl('partytown.js?v=0.8.2')}"><\/script>`;
        ...
        set(url) {
          ...
          if (this.type && config.loadScriptsOnMainThread) {
            const shouldExecuteScriptViaMainThread =
              config.loadScriptsOnMainThread.some((scriptUrl) =>
                new RegExp(scriptUrl).test(url),
              );
              ...
              (() => {
                if (!webWorkerCtx.$initWindowMedia$) {
                  ...
                  webWorkerCtx.$importScripts$(
                    partytownLibUrl('partytown-media.js?v=0.8.2'),
                  );
