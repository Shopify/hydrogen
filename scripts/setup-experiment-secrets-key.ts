...
const secretsPath = resolve(root, "experiments/shared/secrets.ejson");
...
try {
  ...
  process.stderr.write("Verified experiments/shared/secrets.ejson can decrypt.\n");
} catch (error) {
  const message = error instanceof Error ? error.message : String(error);
  process.stderr.write(`Failed to configure experiment secrets key: ${message}\n\n`);
  process.stderr.write(
    "Copy the EJSON private key to your clipboard, then run:\n" +
      "  pnpm run experiments:secrets:setup\n\n" +
