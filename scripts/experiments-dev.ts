#!/usr/bin/env node
// Orchestrates each experiments/* dev server on a freshly-allocated port,
...
// Each project must accept `--port <n>` via its `dev` script. The core
// project's scripts/dev.js wraps `serve` to match the framework dev commands.
async function discoverProjects(): Promise<Project[]> {
  const out: Project[] = [];
  const dirs = await readdir(EXPERIMENTS_DIR, { withFileTypes: true });

  for (const entry of dirs) {
    if (!entry.isDirectory()) continue;

    const name = entry.name;
    const dir = resolve(EXPERIMENTS_DIR, name);
    ...
    try {
      ...
      if (pkg.name && pkg.scripts?.dev) {
        out.push({ name, pkgName: pkg.name });
      }
    } catch {}
  }
  ...
  return new Promise((res, rej) => {
    ...
    s.listen(0, "127.0.0.1", () => {
      ...
      if (address === null || typeof address === "string") {
        rej(new Error("Unable to allocate a project port"));
        return;
      }
...
function spawnProject(ex: Project, port: number): void {
...
async function main(): Promise<void> {
  process.stderr.write("hydrogen experiments — allocating ports\n");
  const projects = await discoverProjects();
  if (!projects.length) {
    process.stderr.write("no projects found under experiments/\n");
    process.exit(1);
  }
  for (const ex of projects) {
...
<title>hydrogen · experiments</title>
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Transparent overlay captures clicks and scroll wheel so the iframe
     can't be interacted with; click opens the project in a new tab. */
  a.frame-link {
