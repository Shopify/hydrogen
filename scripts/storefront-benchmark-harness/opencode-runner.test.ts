...
describe("opencode runner", () => {
  ...
  it("aborts before starting OpenCode when workspace node_modules is not writable", async () => {
    const homePath = await mkdtemp(join(tmpdir(), "opencode-home-"));
    const workspacePath = await mkdtemp(join(tmpdir(), "opencode-workspace-"));
    const summaryOutputPath = join(homePath, "summary.json");
    const nodeModulesPath = join(workspacePath, "node_modules");
    await mkdir(nodeModulesPath);
    await chmod(nodeModulesPath, READ_ONLY_DIRECTORY_MODE);
    ...
    try {
      ...
      await chmod(nodeModulesPath, WRITABLE_DIRECTORY_MODE);
    }

    assert.equal(spawnCallCount, 0);
    const summary = JSON.parse(await readFile(summaryOutputPath, "utf8"));
    assert.equal(summary.ok, false);
    assert.match(summary.error, /not writable/);
  });
});
