...
const WRITABILITY_PROBE_FILE_NAME = ".benchmark-write-probe";
...
export async function runOpenCodeBenchmark({
  ...
  try {
    await assertNodeModulesWritable(workspacePath);
...
async function assertNodeModulesWritable(workspacePath: string): Promise<void> {
  const nodeModulesPath = join(workspacePath, "node_modules");
  const probePath = join(nodeModulesPath, WRITABILITY_PROBE_FILE_NAME);
  try {
    ...
    throw new Error(
      `Benchmark environment is broken: ${nodeModulesPath} is not writable (${formatErrorMessage(error)}). ` +
        "Aborting instead of producing results confounded by the environment.",
    );
  }
}
