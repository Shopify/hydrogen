...
Docker creates that anonymous volume root-owned, so the container entrypoint starts as root, chowns `/workspace/node_modules` to the unprivileged `bench` user, exports `HOME=/home/bench`, and drops privileges via `setpriv` before starting the runner. The runner then probes that `node_modules` is writable and aborts the run with a clear error if it is not — a broken environment should fail the benchmark instead of producing results confounded by permission workarounds.
