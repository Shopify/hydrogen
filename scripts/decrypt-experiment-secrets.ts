...
import { customerAccountConfig, shop, storefrontConfig } from "../experiments/shared/config.ts";

const __dirname = dirname(fileURLToPath(import.meta.url));
const root = resolve(__dirname, "..");
const sharedDir = resolve(root, "experiments/shared");
const ejsonFile = "secrets.ejson";
// These markers still say "example" on purpose: they delimit blocks this script
// upserts into every developer's existing local .env files. Renaming them would
// orphan those blocks and duplicate the env vars on the next run.
...
try {
  ...
  process.stderr.write(
    `Failed to decrypt experiments/shared/${ejsonFile}: ${message}\n\n` +
      "Local development:\n" +
      "  Configure ejson's keydir, then run pnpm run experiments:secrets:decrypt\n\n" +
      "CI environment:\n" +
      "  Set EJSON_PRIVATE_KEY, then run pnpm run experiments:secrets:decrypt\n",
  );
  process.exit(1);
}
...
function getEnvSecrets(secrets: Record<string, string>): Record<string, string> {
  ...
  if (!privateStorefrontToken) {
    throw new Error(
      `Missing ${storefrontConfig.privateStorefrontTokenEnvKey} in experiments/shared/${ejsonFile}`,
    );
  }
...
function getExampleEnvFileTargets(): EnvFileTarget[] {
  const projectsDir = resolve(root, "experiments");
  const envFileTargets: EnvFileTarget[] = [];

  for (const entry of readdirSync(projectsDir, { withFileTypes: true })) {
    if (!entry.isDirectory() || entry.name === "shared") continue;

    const packageJsonPath = resolve(projectsDir, entry.name, "package.json");
    ...
    if (packageJson.scripts?.dev) {
      envFileTargets.push(createEnvFileTarget(resolve(projectsDir, entry.name)));
    }
  }
